...
class RotatedLabel extends google.maps.OverlayView {	
	...
	constructor(position, text, rotation, map, customStyles = {}, offsetX = 0, offsetY = 0) {
		super();
	...
	moveOverlayDiv() {
		...
		const div = this.div_;
		div.style.left = (divPixel.x + this.offsetX_) + 'px';
		div.style.top = (divPixel.y + this.offsetY_) + 'px';
	}

	/*
	moveOverlayDivBy(offsetX = 0, offsetY = 0) {
		this.overlayProjection = this.getProjection();
		const divPixel = this.overlayProjection.fromLatLngToDivPixel(this.position_);

		const div = this.div_;
		console.log(div.style);
		this.div_.left = (divPixel.x + offsetX) + 'px';
		div.style.top = (divPixel.y + offsetY) + 'px';
	}*/
